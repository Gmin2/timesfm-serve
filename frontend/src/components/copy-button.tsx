...
export function CopyButton({ value, label }: { value: string; label: string }) {
  ...
  }}>{state === 'copied' ? <Icon.Check /> : <Icon.Copy />}</Button><span className="sr-only" role="status">{state === 'copied' ? 'Copied' : state === 'failed' ? 'Clipboard unavailable. Select the text to copy.' : ''}</span></span>
}
